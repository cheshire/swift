...
Swift compiler comes with builtin `libFuzzer` integration.
Currently, the library usage requires writing an Objective-C bridge boilerplate.
Suppose we would like to fuzz-test a Swift function called `fuzzMe` from the
file `myfile.swift`.
First, we have to put (or call the function from) an `@objc` block within the
Swift class:

.. code-block:: swift

    @objc(MyClass) class MyClass {
    static func fuzzMe(Data: UnsafePointer<CChar>) {
      // Test our code using provided Data.
    }
    }

Then, we have to call `[MyClass fuzzMeWithData]` (note the function name
includes an auto-generated `WithData` postfix; use the `-emit-objc-header` flag
for better integration) from an entry point function inside
a separate Objective-C file, which we shall call `fuzzwrapper.m`:
...
.. code-block:: objective-c
    ...
    int LLVMFuzzerTestOneInput(const char *Data, long Size) {
      [MyClass fuzzMeWithData:Data];
    }

Finally, we have to compile and link the two files together using
`-sanitize=fuzzer` flag for Swift, and `-fsanitize=fuzzer` flag for Clang:

.. code-block:: bash

    % swiftc -sanitize=fuzzer -parse-as-c-library -c myfile.swift -o myfile.o
    % clang -fsanitize=fuzzer -c fuzzwrapper.m -o fuzzwrapper.o
    % swiftc -sanitize=fuzzer fuzzwrapper.o myfile.o -o fuzztarget

Note the usage of `-parse-as-c-library` flag: otherwise, Swift inserts its
own `main` symbol, which does not let `libFuzzer` execute.

Finally, we can just launch the fuzzing process:
